...
export async function getFinancialAccountControl({
  ...
  // or= em quatro colunas: nome, CNPJ/CPF, nº documento e assunto do e-mail
  if (supplier) {
    url.searchParams.set(
      'or',
      `(supplier_name.ilike.*${supplier}*,supplier_cnpj.ilike.*${supplier}*,invoice_number.ilike.*${supplier}*,subject.ilike.*${supplier}*)`,
    );
  }
