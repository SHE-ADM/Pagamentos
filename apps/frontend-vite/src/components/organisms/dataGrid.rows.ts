...
// exatamente UM `<tr>`: a linha principal, a sub-linha responsiva (second) e o
// painel de detalhe (detail). Isso permite virtualizar por item (1 tr = 1 índice
// medível), resolvendo o problema de uma linha de dados emitir múltiplos `<tr>`.
// Função pura (sem dependência do TanStack/React) — fácil de testar isoladamente.

type RenderItemKind = 'row' | 'second' | 'detail';
 ...
 * linha principal → sub-linha (se houver) → detalhe (se selecionada e há `renderDetail`).
