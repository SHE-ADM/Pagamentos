...
// Atom — campo de formulário com label, fundo preenchido e estado de foco.
// Aceita `endAdornment` (ex.: botão olho) renderizado à direita do input.
export default function FilledTextField({ label, error, endAdornment, ...inputProps }) {
  const [focused, setFocused] = useState(false)

  return (
    <div className="flex flex-col gap-2">
      <label className="text-sm font-bold text-loginGreen-ink">
        {label}
      </label>
      <div className={`flex items-center h-12 px-3.5 gap-2.5 rounded-lg border-2 transition-colors
        ${focused
