...
import { getProcessingErrors, getProcessingErrorStats } from '../services/supabase'
import StatusBadge from '../components/StatusBadge'

const fmtDt = d => d ? new Date(d).toLocaleString('pt-BR') : '—'

const ERROR_TYPES = [
  'sem_valor', 'sem_fornecedor', 'extracao_falhou', 'db_erro', 'processamento_erro', 'pdf_protegido'
]
...
export default function Erros() {
  ...
  const [f, setF_]      = useState({ errorType: '', sender: '', dateFrom: '', dateTo: '' })
  const [applied, setApplied] = useState({ errorType: '', sender: '', dateFrom: '', dateTo: '' })
    ...
    <div className="flex flex-col h-full">
      <div className="px-6 py-4 border-b border-gray-200 bg-white flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-gray-500 mt-0.5">Registros com falha — tabela email_processing_errors</p>
        </div>
      ...
      <div className="flex-1 overflow-y-auto px-6 py-5">
        ...
        <div className="grid grid-cols-5 gap-3 mb-5">
          {[
            ...
            { label: 'Sem fornecedor',  value: stats.counts.sem_fornecedor  ?? 0,    icon: AlertTriangle, cls: 'text-orange-500' },
            { label: 'Extração falhou', value: stats.counts.extracao_falhou ?? 0,    icon: XCircle,       cls: 'text-red-500'    },
            { label: 'DB / processo',   value: (stats.counts.db_erro ?? 0) + (stats.counts.processamento_erro ?? 0), icon: Info, cls: 'text-purple-500' },
        ...
        <div className="flex gap-2 mb-4 flex-wrap">
          ...
          <input className="input w-44" placeholder="Remetente…"
            value={f.sender} onChange={e => sf('sender', e.target.value)}
            onKeyDown={e => e.key === 'Enter' && handleSearch()} />
          <input type="date" className="input w-36" value={f.dateFrom}
            onChange={e => sf('dateFrom', e.target.value)} title="Data de" />
          <input type="date" className="input w-36" value={f.dateTo}
            onChange={e => sf('dateTo', e.target.value)} title="Data até" />
            ...
            <tbody>
              {rows.length === 0 ? (
                ...
                <tr key={r.id}
                  ...
                  <td className="table-cell"><StatusBadge value={r.error_type} /></td>
                  <td className="table-cell text-xs max-w-[140px] truncate" title={r.sender_email}>{r.sender_email || '—'}</td>
                  <td className="table-cell text-xs max-w-[180px] truncate" title={r.subject}>{r.subject || '—'}</td>
                  <td className="table-cell text-xs font-mono text-gray-500 max-w-[120px] truncate" title={r.source_file}>{r.source_file || '—'}</td>
                  <td className="table-cell text-xs max-w-[200px] truncate text-red-600" title={r.error_message}>{r.error_message || '—'}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <div className="flex items-center justify-between py-2 px-1 mb-4">
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => goPage(page - 1)} disabled={page <= 1 || loading} className="btn">← Anterior</button>
            <button onClick={() => goPage(page + 1)} disabled={page >= totalPages || loading} className="btn">Próxima →</button>
          </div>
        </div>
