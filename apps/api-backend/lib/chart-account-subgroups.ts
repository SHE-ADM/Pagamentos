...
const repository = {
  async findAll(params: { from: number; to: number; search?: string; sort?: string; order?: SortOrder }) {
    ...
    const fallback = { column: 'subgroup_code', ascending: true, nullsFirst: false };

    return applyOrder(query, sorted, fallback, 'chart_account_subgroup_id').range(params.from, params.to);
  },
