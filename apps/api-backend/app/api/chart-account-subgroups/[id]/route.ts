...
export async function GET(req: NextRequest, ctx: Context) {
  ...
  if (id === null) return fail('Identificador de subgrupo inválido', 400);
  try {
    return ok(await chartAccountSubgroupService.getById(id));
  } catch (e) {
    return mapError(e);
  }
}
...
export async function PATCH(req: NextRequest, ctx: Context) {
  ...
  try {
    return ok(await chartAccountSubgroupService.update(id, body ?? {}));
  } catch (e) {
    return mapError(e);
  }
}
...
export async function DELETE(req: NextRequest, ctx: Context) {
  ...
  if (id === null) return fail('Identificador de subgrupo inválido', 400);
  try {
    return ok(await chartAccountSubgroupService.remove(id));
  } catch (e) {
    return mapError(e);
  }
}
