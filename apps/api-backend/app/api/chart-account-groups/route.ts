...
async function listPaginated(sp: URLSearchParams): Promise<Response> {
  ...
  try {
    const r = await chartAccountGroupService.list({
...
async function listForLookup(sp: URLSearchParams): Promise<Response> {
  try {
    const r = await chartAccountGroupService.list({ limit: LOOKUP_LIMIT, search: sp.get('search') ?? undefined });
    return ok(r.data);
  } catch (e) {
    return mapError(e);
  }
...
export async function POST(req: NextRequest) {
  ...
  try {
    return ok(await chartAccountGroupService.create(body ?? {}), undefined, 201);
  } catch (e) {
    return mapError(e);
  }
}
