...
│   ├── BreakdownDonut.tsx     #   donut genérico (conic-gradient + furo + legenda), prop {segs:{key,label,count,value}[], colorFor}. Arcos + % e ORDEM das fatias por VALOR (R$) desc (não por contagem); legenda exibe R$ (fmtMoney) + contagem + %; furo central mostra a CONTAGEM total de contas
