...
│   └── ExpenseDetailModal.tsx (+ .test.tsx, .a11y.test.tsx)  #   card de DETALHE (drill-down) de /dashboard_despesas: modal <dialog> centralizado + DataGrid enxuto (getExpenseDetailColumns: Fornecedor/Plano de conta/Vencimento/Valor) das contas da fatia/linha clicada. Rows já filtradas pela página (filterExpenseDetailRows); ordena por valor + rodapé total. Ver "Card de DETALHE (drill-down)" na rota /dashboard_despesas
...
> `extract_pdf.py`** — copiar `skills/pdf-contas-pagar/scripts/extract_pdf.py` para produção. O
> `read_emails.py` **não muda nesta**, mas ele carrega deltas ANTERIORES ainda pendentes (3ª
> empresa etc.); na dúvida, copie **os 2 juntos** (são interdependentes in-process). Sem `.env`,
> sem migration. A conta histórica afetada (575) já foi recuperada (id 593) via `reprocess_message`.
