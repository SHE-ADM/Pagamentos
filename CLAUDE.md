...
│   ├── BreakdownDonut.tsx     #   donut genérico (conic-gradient + furo + legenda), prop {segs:{key,label,value}[], colorFor, dense?}. Arcos + % + ORDEM das fatias por VALOR (R$) desc; legenda = R$ (fmtMoney) + % (SEM contagem de contas); furo central = TOTAL em R$ compacto (fmtMoneyCompact, ex.: "R$ 12,3 mil") rotulado "total". `dense` (usado em /dashboard_vencimentos): fonte menor (donut-legend-dense 0.65rem), círculo 92px e menos espaçamento — cabe mais info
