...
$SCRIPT       = Join-Path $PROJECT_ROOT "skills\baixa-automatica\scripts\run.py"
$LOG_DIR      = Join-Path $PROJECT_ROOT "logs\baixa"
...
# Só rejeita o build free-threaded (3.14t) por precaução de paridade com as demais
# tarefas; a baixa usa apenas urllib (stdlib) e python-dotenv.
...
$LOG_FILE   = Join-Path $LOG_DIR "baixa_$(Get-Date -Format 'yyyyMMdd').log"
...
Get-ChildItem -Path $LOG_DIR -Filter "baixa_*.log" |
...
if ($DryRun) { Write-Log "Modo   : DRY-RUN (nao grava)" }
...
# Grava stderr no log diário com marcação clara (o logging INFO do Python sai por stderr —
# aparece mesmo em sucesso; o sinal de erro é o exit != 0, não a presença de stderr).
...
if ($EXIT -eq 0) {
    ...
    try {
        $src = "Pagamentos-Baixa"
        if (-not [System.Diagnostics.EventLog]::SourceExists($src)) {
            [System.Diagnostics.EventLog]::CreateEventSource($src, "Application")
        }
        Write-EventLog -LogName Application -Source $src -EventId 1004 `
            -EntryType Error `
            -Message "run.py (baixa automatica) terminou com exit code $EXIT em $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss'). Verifique $LOG_DIR"
    } catch {
