...
| Item | Estado |
|---|---|
| Último deploy | **2026-09-23** — carnê RAINHA MARIA (isenção por título próprio/código descartado) · boleto prorrogado (`barcode_due_date_supersedes`, teto 60 dias) · DV geral no Vision · dedup não funde irmãos descartados · nota "Vencimento corrigido" preservada na gravação. Arquivos copiados **juntos**: `febraban.py`, `extract_pdf.py`, `read_emails.py`, `deploy-manifest.json`. Migrations 143–146 **já aplicadas**. PR #255, merge `86595ab` — manifesto `EE982709…73E21` |
| Paridade verificada | ✅ **em produção** (2026-09-23) — `check_deploy_parity.py`: **32/32 conferem, 0 faltando, 0 divergentes, 0 extras**. ⏳ Validação funcional (`prorrogacao`, `nota_preservada`, `isencao_carne`) proposta, não reportada · sinal no dado a observar: `pagavel_descartado` = 0 e uma conta por anexo no próximo lote da RAINHA MARIA |
...
| RAINHA MARIA: 6 parcelas de carnê perdidas em silêncio + 6 vencimentos de boleto PRORROGADO (2026-09-22) | dados ✅ (migrations 143, 144 e 145 aplicadas) · código ✅ · ✅ **pipeline em produção (2026-09-23, PR #255)** | Quatro defeitos num levantamento só: (1) num carnê o VALOR não discrimina e a regra fatura+boleto apagava as parcelas sem código íntegro — **R$ 150.552,00**, e-mail `extraído`, nada em `/erros`; (2) o fator do barcode sobrescrevia a data IMPRESSA de boleto **prorrogado**, e a rede do `register_financial`, sendo a última a falar, revertia o extrator (a conta **1029** já tinha sido corrigida à mão em 14/08); (3) no visual o modelo converte a linha de 47→44 e erra o campo livre — **20 códigos** na base, 100% `pdf_vision`; (4) o builder apagava a nota da própria correção. O **review max** (mesmo dia) achou 3 bloqueantes na própria correção — todos corrigidos: o descarte de código no Vision reabria a perda pela 3ª impressão da dedup (hoje o descarte é MARCADO e a marca vira `barcode=is.null`), o vencimento PRESUMIDO virava "data impressa" (hoje o fator vence sempre ali) e o descarte rodava ANTES da derivação pelo fator (hoje roda no fim da cadeia). Teto de prorrogação em **60 dias**. Relatório: [docs/review/2026-09-22-Features-max.md](docs/review/2026-09-22-Features-max.md). O **2º review max (2026-09-23)** achou a correção da dedup incompleta: irmãos **ambos** descartados (barcode nulo nos dois) ainda se fundiam — lote 1582/1583/1584, mesmo e-mail —, hoje com veto por título distinto; e a gravação apagava a nota "Vencimento corrigido", hoje preservada. Relatório: [docs/review/2026-09-23-Features-max.md](docs/review/2026-09-23-Features-max.md). Copiar para produção, **os quatro juntos** (`read_emails.py` novo com `febraban.py` antigo levanta `TypeError` na gravação): `febraban.py`, `extract_pdf.py`, `read_emails.py`, `deploy-manifest.json`. Sinal no dado: `SELECT count(*) FROM email_processing_errors WHERE error_type='pagavel_descartado'` deve seguir **0** (dead-man switch). Detalhe em [docs/knowledge/pipeline-extracao.md](docs/knowledge/pipeline-extracao.md) |
| 16 barcodes corrompidos históricos (`pdf_vision`) | ✅ **RESOLVIDO em 2026-09-23** (migration 146) | Os 16 receberam a linha digitável **impressa**, lida do PDF original e validada por DV + valor + fator; a sonda P6 é o oráculo do acervo — nenhuma conta da base segue com DV de boleto refutado. A dívida é FECHADA: desde 2026-09-22 o pipeline descarta o código refutado em vez de gravá-lo. ⚠️ Fica um achado NÃO corrigido: 9 dessas contas (todas pagas) têm `due_date` divergente do vencimento impresso no boleto — mesma causa, decisão sua se vale mexer em conta fechada. Conferir: `SELECT count(*) FROM financial_account_control WHERE barcode IS NOT NULL` com o teste de DV deve dar 0 |
| AMIL: boleto de outubro reescreveu a conta PAGA de julho (conta 417 — 2026-09-23) | código ✅ no repositório · dados ✅ (corrigidos em 2026-09-24) · ⏳ **produção aguardando cópia manual** | O "Nº do Documento" Amil é o nº do CONTRATO (`003071000`), igual todo mês; a impressão 2 casou o boleto de 07/10 com a 417 (07/07, paga, manual, sem nosso número) e a reemissão a reescreveu. Hoje conta quitada + vencimento posterior = dívida NOVA, e a dedup refaz a busca vetando a quitada (`skip_settled`). O **review max (2026-09-24)** achou que a 1ª versão parava a busca na quitada e escondia a conta do corpo da mesma dívida (2 contas em aberto) — corrigido. Relatório: [docs/review/2026-09-24-Features-max.md](docs/review/2026-09-24-Features-max.md). **Correção de dados, sem migration:** a 417 voltou a 07/07 e sem barcode por UPDATE de serviço (`audit_log` 2026-09-24 13:08:02 UTC) e o e-mail 2382 foi reprocessado pelo pipeline completo, criando a **1680** (07/10, em aberto, com a nota "conta 417 (quitada…)", 13:08:25 UTC). Conferir: `SELECT id, due_date, status_id, barcode IS NULL FROM financial_account_control WHERE id IN (417,1680)` → 417 = 2026-07-07/8/true; 1680 = 2026-10-07/aberta/false. Copiar para produção **os dois juntos**: `read_emails.py`, `deploy-manifest.json` (sem módulo nem dependência nova). Sinal no dado: o boleto Amil de novembro deve gerar conta própria com a nota, e a 417 não pode aparecer em `audit_log` de novo. ⚠️ Residual: o caminho do CORPO não aplica a regra (sem ocorrência medida). Detalhe em [docs/knowledge/pipeline-extracao.md](docs/knowledge/pipeline-extracao.md) |
