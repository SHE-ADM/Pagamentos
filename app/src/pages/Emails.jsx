    ...
    <div className="flex flex-col h-full">
      <div className="px-6 py-4 border-b border-gray-200 bg-white flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-gray-500 mt-0.5">Monitoramento da caixa IMAP — registros em email_control</p>
        </div>
      ...
      <div className="flex-1 overflow-y-auto px-6 py-5">
        ...
        <div className="grid grid-cols-4 gap-3 mb-5">
          {[
            ...
            { icon: CopyMinus,  label: 'Só recebidos',      value: (stats.total ?? 0) - (stats.extracted ?? 0) - (stats.semPdf ?? 0), sub: 'aguardando' },
        ...
        <div className="flex gap-2 mb-4">
          <input className="input max-w-xs" placeholder="Remetente ou assunto…"
            ...
            <tbody>
              {rows.length === 0 ? (
                ...
                <tr key={r.id}
                  ...
                  <td className="table-cell text-xs max-w-[140px] truncate" title={r.sender_email}>{r.sender_name || r.sender_email}</td>
                  <td className="table-cell text-xs max-w-[200px] truncate" title={r.subject}>{r.subject}</td>
                  <td className="table-cell"><StatusBadge value={r.keyword_matched} /></td>
                  <td className="table-cell text-center">{r.has_attachment ? '✓' : <span className="text-gray-300">—</span>}</td>
                  <td className="table-cell">{r.pdf_extracted ? <StatusBadge value="extracted" /> : <span className="text-xs text-gray-400">—</span>}</td>
        ...
        {sel && (
          <div className="card p-4">
            <p className="text-xs font-semibold text-gray-500 mb-3 uppercase tracking-wide">Detalhes do e-mail selecionado</p>
